...
- 缓存清理 confirm / running / success / error。

- 可清理进程列表 loading / empty / error。
...
|1\.2|`GET /api/system/resources`|本机资源概览实时刷新|`diskId`、`includeProcessCleanupEstimate`|CPU、GPU、内存、磁盘、网络、可释放内存估算|
|1\.3|`GET /api/activity/recent`|最近操作|`limit`、`projectId`|`items[].resourceType`、`resourceId`、`tone`|
|1\.4|`POST /api/system/cache/cleanup`|执行加速缓存清理|请求头 `Idempotency-Key`；`confirmed=true`|`cleanupId`、`cleanedBytes`、`cleanedFileCount`、`skipped`|
|1\.5|`GET /api/system/processes`|查询可清理进程列表|`sortBy`、`limit`、`includeProtected`|`selectionToken`、`expiresAt`、`releasableMemoryBytes`、`items[]`|
|1\.6|`POST /api/system/processes/terminate`|终止勾选进程|请求头 `Idempotency-Key`；`confirmed=true`、`selectionToken`、`pids`|`operationId`、`results[]`、`finishedAt`|
|1\.7|`POST /api/local-tools/open`|本地工具启动器|`tool`、`projectId`、`path`|`opened`、`tool`、`path`|
...
- 加速缓存清理直接调用后端内置受控缓存清理接口，不接收前端传入任意路径，必须传 `confirmed=true` 和 `Idempotency-Key`。

- 进程清理必须先调用 `GET /api/system/processes` 获取 `selectionToken`，用户勾选进程并确认后再调用 `POST /api/system/processes/terminate`；前端不得自行构造未出现在列表快照中的 PID。
...
|4|1\.4|`POST /api/system/cache/cleanup`|
|5|1\.5|`GET /api/system/processes`|
|6|1\.6|`POST /api/system/processes/terminate`|
|7|1\.7|`POST /api/local-tools/open`|
|8|2\.1|`GET /api/projects`|
|9|2\.2|`GET /api/projects/specs`|
|10|2\.3|`POST /api/projects`|
|11|2\.4|`GET /api/projects/{projectId}`|
|12|2\.5|`DELETE /api/projects/{projectId}`|
