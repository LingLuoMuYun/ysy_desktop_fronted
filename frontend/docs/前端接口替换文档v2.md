...
|4\.1 / 3\.1 / 5\.1 / 6\.2|关联资源列表与默认环境|项目详情 Tab|`projectId`、`envId`|`GET /api/datasets?projectId=...`、`GET /api/tasks?projectId=...`、`GET /api/models?projectId=...`、`GET /api/environments/{envId}`|
...
|后端章节|后端接口|前端替换位置|输入/参数|输出关注|
|---|---|---|---|---|
|6\.1|`GET /api/environments`|环境列表、筛选、分页|`keyword`、`purpose`、`status`、`page`、`pageSize`|分页环境列表|
|6\.2|`GET /api/environments/{envId}`|环境详情抽屉|`envId`|Python、框架、CUDA、检测结果、最近项目|
|6\.3|`POST /api/environments`|创建系统/自定义环境|`mode`、`name`、`purpose`、`template`、`python`、`cuda`、`savePath`、`autoCheck`|新环境对象|
|6\.4|`POST /api/environments/import`|导入本地环境|`name`、`pythonPath`、`purpose`、`autoCheck`|导入后的环境对象|
|6\.5|`POST /api/environments/{envId}/check`|检测环境|`envId`|`CheckResult`：`status`、`summary`、`items`、`checkedAt`、`source`、`warnings`|
|6\.6|`POST /api/environments/batch-check`|检测全部环境|`purpose`、`includeUnavailable`|`jobId`、`status`、`total`、`checked`、`items[].checkResult`、`failedEnvironmentIds`|
|6\.6|`GET /api/environments/batch-check/{jobId}`|查询批量检测进度|`jobId`|`status`、`checked`、`items[].checkResult`|
|6\.7|`POST /api/environments/{envId}/repair`|修复环境|`confirmed=true`、`strategy`|`repairTaskId`、`repairStatus`|
|6\.8|`DELETE /api/environments/{envId}`|删除环境|`confirmed=true`、`deleteLocalFiles=false`|`deleted`|
...
|后端章节|后端接口|前端替换位置|输入/参数|输出关注|
|---|---|---|---|---|
|7\.1|`GET /api/settings/profile`|查询个人信息|无|`accountId`、`email`、`phone`|
|7\.2|`PATCH /api/settings/profile`|保存个人信息|`accountId`、`email`、`phone`|更新后的个人信息|
|7\.3|`GET /api/settings/preferences`|查询偏好配置|无|`theme`、`language`、`defaultWorkspace`、`confirmPolicy`|
|7\.4|`PATCH /api/settings/preferences`|保存偏好配置|`theme`、`language`、`defaultWorkspace`、`confirmPolicy`|更新后的偏好配置|
...
- 批量检测环境是低风险读检查；若返回 `jobId`，前端轮询 `GET /api/environments/batch-check/{jobId}`。
...
|47|6\.1|`GET /api/environments`|
|48|6\.2|`GET /api/environments/{envId}`|
|49|6\.3|`POST /api/environments`|
|50|6\.4|`POST /api/environments/import`|
|51|6\.5|`POST /api/environments/{envId}/check`|
|52|6\.6|`POST /api/environments/batch-check`|
|53|6\.6|`GET /api/environments/batch-check/{jobId}`|
|54|6\.7|`POST /api/environments/{envId}/repair`|
|55|6\.8|`DELETE /api/environments/{envId}`|
|56|7\.1|`GET /api/settings/profile`|
|57|7\.2|`PATCH /api/settings/profile`|
|58|7\.3|`GET /api/settings/preferences`|
|59|7\.4|`PATCH /api/settings/preferences`|
|60|8\.1|`POST /api/local-files/pick-directory`|
|61|8\.2|`POST /api/local-files/validate-path`|
