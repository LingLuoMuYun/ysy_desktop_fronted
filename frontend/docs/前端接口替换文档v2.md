...
|后端章节|后端接口|前端替换位置|输入/参数|输出关注|
|---|---|---|---|---|
|2\.1|`GET /api/projects`|项目列表、筛选、分页|`keyword`、`type`、`status`、`page`、`pageSize`；`status` 使用英文 code|`items`、`total`、`hasMore`；项目状态按 `status` / `statusText` 分离|
|2\.2|`GET /api/projects/specs`|创建项目表单初始化|无|`projectTypes`、`statusOptions`、`workspaceOptions`、`compatibleEnvironments`|
|2\.3|`POST /api/projects`|创建项目|`name`、`type`、`description`、`workspace`、`environmentId`、`confirmed=true`|新项目 ID、`status`、`statusText`、默认环境信息|
|2\.4|`GET /api/projects/{projectId}`|项目详情基础信息|`projectId`|基础信息、`datasetCount`、`taskCount`、`modelCount`、`environmentId`、`environmentName`、`summary`|
|4\.1 / 3\.1 / 5\.1|关联资源列表|项目详情 Tab|`projectId`|`GET /api/datasets?projectId=...`、`GET /api/tasks?projectId=...`、`GET /api/models?projectId=...`；默认环境直接展示项目详情返回的 `environmentName`|
|2\.5|`DELETE /api/projects/{projectId}`|删除项目|`confirmed=true`、`deleteLocalFiles=false`|`deleted`、`projectId`|

### 边界限制

- 项目名称重复或路径不可写时展示字段级错误。

- 创建项目接口不再接收代码来源、代码路径、依赖文件或初始数据集绑定；训练入口、启动命令、输出目录和数据集选择进入后续任务配置或资产绑定流程。

- 创建项目属于中风险操作，提交前需要展示创建摘要，最终请求必须传 `confirmed=true`。

- 项目状态使用 code / 文案分离：筛选、排序、业务判断使用 `status`；状态标签展示使用 `statusText`，并在前端维护 `status` 到 tone 的映射。

- 删除项目默认不删除本地文件；MVP 阶段 `deleteLocalFiles` 只允许为 `false`，前端应隐藏或禁用“删除本地文件”选项，传 `true` 时后端返回 `future_not_supported` 或 `validation_error`。

- 项目详情基础接口只返回计数、默认环境 ID、默认环境名称和摘要；关联数据、任务、模型列表不由项目详情内嵌返回，完整列表、筛选、分页应回到数据、任务、模型模块接口。

- 关联数据集或环境不存在时，需要提示重新绑定。

- 编辑项目接口在新版后端项目章节中未列出，前端不得按旧版 `PATCH /api/projects/{projectId}` 落地编辑能力；是否恢复编辑接口待后端确认。
...
|11|2\.5|`DELETE /api/projects/{projectId}`|
|12|3\.1|`GET /api/tasks`|
|13|3\.2|`GET /api/tasks/specs`|
|14|3\.3|`POST /api/tasks/preflight`|
|15|3\.4|`POST /api/tasks`|
|16|3\.5|`GET /api/tasks/{taskId}`|
|17|3\.6|`GET /api/tasks/{taskId}/config`|
|18|3\.7|`GET /api/tasks/{taskId}/logs`|
|19|3\.8|`GET /api/tasks/{taskId}/outputs`|
|20|3\.9|`POST /api/tasks/{taskId}/reports`|
|21|3\.9|`GET /api/tasks/{taskId}/reports`|
|22|3\.9|`GET /api/reports/{reportId}`|
|23|3\.10|`POST /api/tasks/{taskId}/start`|
|24|3\.11|`POST /api/tasks/{taskId}/stop`|
|25|3\.12|`DELETE /api/tasks/{taskId}`|
|26|3\.13|`POST /api/tasks/{taskId}/clone`|
|27|3\.13|`POST /api/tasks/{taskId}/retry`|
|28|3\.14|`POST /api/tasks/{taskId}/register-model`|
|29|3\.15|`GET /api/tasks/{taskId}/test-spec`|
|30|3\.16|`POST /api/tasks/{taskId}/test`|
|31|4\.1|`GET /api/datasets`|
|32|4\.2|`POST /api/datasets`|
|33|4\.3|`GET /api/datasets/{datasetId}`|
|34|4\.4|`GET /api/datasets/{datasetId}/profile`|
|35|4\.5|`POST /api/datasets/{datasetId}/check`|
|36|4\.6|`GET /api/datasets/{datasetId}/check-result`|
|37|4\.7|`PATCH /api/datasets/{datasetId}/projects`|
|38|4\.8|`DELETE /api/datasets/{datasetId}`|
|39|5\.1|`GET /api/models`|
|40|5\.2|`POST /api/models`|
|41|5\.3|`GET /api/models/{modelId}`|
|42|5\.4|`POST /api/models/{modelId}/check`|
|43|5\.5|`GET /api/models/{modelId}/check-result`|
|44|5\.6|`PATCH /api/models/{modelId}/projects`|
|45|5\.7|`DELETE /api/models/{modelId}`|
|46|7\.1|`GET /api/environments`|
|47|7\.2|`POST /api/environments`|
|48|7\.3|`POST /api/environments/import`|
|49|7\.4|`POST /api/environments/{envId}/check`|
|50|7\.5|`POST /api/environments/{envId}/repair`|
|51|7\.6|`DELETE /api/environments/{envId}`|
|52|8\.1|`GET /api/settings/profile`|
|53|8\.2|`PATCH /api/settings/profile`|
|54|8\.3|`GET /api/settings/preferences`|
|55|8\.4|`PATCH /api/settings/preferences`|
|56|9\.1|`POST /api/local-files/pick-directory`|
|57|9\.2|`POST /api/local-files/validate-path`|
