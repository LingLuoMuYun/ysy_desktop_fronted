...
4. 项目列表每页展示 5 个项目，底部展示当前页、总页数和总项目数。
...
- 项目列表请求固定提交 `pageSize=5`；翻页时提交目标 `page`。
- 项目列表底部展示“第 x 页 / 共 y 页，共 z 个项目”。`y` 优先使用响应 `totalPages`；响应未提供时按 `Math.ceil(total / 5)` 计算，`total=0` 时总页数展示为 0 或按空态隐藏分页控件。
