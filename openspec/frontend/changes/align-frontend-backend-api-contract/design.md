...
5. 项目工作区路径提交前必须调用路径校验接口。
6. 项目编辑入口先预留一个空按钮，不触发请求。
7. 项目详情页关联数据、任务、模型 Tab 本阶段保留空态。
...
- 项目列表从 mock 替换为项目服务层。
- 创建项目弹窗接入规格接口和创建接口。
- 项目详情接入详情接口；关联数据、任务、模型 Tab 保留空态。
- 项目编辑入口预留一个空按钮，不提交请求。
...
- 项目详情中的关联数据、任务、模型列表复用其他模块接口；当前阶段保留空态，不接列表接口。
- 项目创建工作区必须调用路径校验接口；本地目录选择入口可继续复用现有前端能力。
